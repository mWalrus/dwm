...
struct Client {
	...
	// FIXME: moving the cursor between windows moves the cursor and the focus back
	//        this is most likely due to focus handling when it comes to steam apps.
	//        Investigate...
	int issteam;
	Client *next;
	Client *snext;
...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* status is only drawn on selected monitor */
		...
		tw = TEXTW(stext);
		drw_text(drw, m->ww - tw, 0, tw, bh, lrpad / 2, vertpadbar / 2, stext, 0);
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		drw_text(drw, x, 0, w, bh, lrpad / 2, vertpadbar / 2, tags[i], urg & 1 << i);
		if (occ & 1 << i)
	...
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, vertpadbar / 2, m->ltsymbol, 0);

	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, vertpadbar / 2, m->sel->name, 0);
			if (m->sel->isfloating)
